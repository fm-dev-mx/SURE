<?php
	...
	class consultaControlador extends consultaModelo{
						...
						if($ec>=1){
							...
							if($consulta3->rowCount()>=1){
								...
								if($guardarCuenta->rowCount()>=1){
									...
									$guardarAdmin=consultaModelo::agregar_consulta_modelo($dataAD);

									if($guardarAdmin->rowCount()>=1){
										$alerta=[
											"Alerta"=>"limpiar",
											"Titulo"=>"¡consulta registrado!",
											"Texto"=>"El consulta se registró con éxito en el sistema",
											"Tipo"=>"success"
										];
									}else{
										...
										$alerta=[
											"Alerta"=>"simple",
											"Titulo"=>"Ocurrió un error inesperado",
											"Texto"=>"No hemos podido registrar el consulta",
											"Tipo"=>"error"
										];
									}
								}else{
		...
		// Controlador para paginar consultaes
		public function paginador_consulta_controlador($pagina,$registros,$privilegio,$codigo,$busqueda){
					...
					$tabla.='
						<tr>
							...
							if($privilegio==1){
								$tabla.='
									<td>
										<form action="'.SERVERURL.'ajax/consultaAjax.php" method="POST" class="FormularioAjax" data-form="delete" entype="multipart/form-data" autocomplete="off">
		...
		public function eliminar_consulta_controlador(){
			...
			if($adminPrivilegio==1){
				...
				if($datosAdmin['id']!=1){
					$DelAdmin=consultaModelo::eliminar_consulta_modelo($codigo);
					...
					if($DelAdmin->rowCount()>=1){
						...
						if($DelCuenta->rowCount()==1){
							$alerta=[
								"Alerta"=>"recargar",
								"Titulo"=>"consulta eliminado",
								"Texto"=>"El consulta fue eliminado del sistema con éxito",
								"Tipo"=>"success"
							];
						}else{
							$alerta=[
						...
						$alerta=[
							"Alerta"=>"simple",
							"Titulo"=>"Ocurrió un error inesperado",
							"Texto"=>"No podemos eliminar este consulta en este momento",
							"Tipo"=>"error"
						];
					}
				}else{
					$alerta=[
						"Alerta"=>"simple",
						"Titulo"=>"Ocurrió un error inesperado",
						"Texto"=>"No podemos eliminar el consulta principal del sistema",
						"Tipo"=>"error"
					];
				}
			}else{
		...
		public function datos_consulta_controlador($tipo,$codigo){
			$codigo=mainModel::decryption($codigo);
			$tipo=mainModel::limpiar_cadena($tipo);

			return consultaModelo::datos_consulta_modelo($tipo,$codigo);
		}
		...
		public function actualizar_consulta_controlador(){
			...
			if(consultaModelo::actualizar_consulta_modelo($dataAd)){
				$alerta=[
					"Alerta"=>"recargar",
